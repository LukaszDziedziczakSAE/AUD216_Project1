...
public class InteractionAudioManager : MonoBehaviour
{
    ...
    public void PlayMixingDeskAudio(string profileName, bool play)
    {
        ...
        // Validate required components with detailed logging
        if (profile.leftSpeakerAudio == null)
            Debug.LogError("Left speaker audio clip is missing");
        if (profile.rightSpeakerAudio == null)
            Debug.LogError("Right speaker audio clip is missing");
        if (profile.leftSpeakerSource == null)
            Debug.LogError("Left speaker AudioSource is missing");
        if (profile.rightSpeakerSource == null)
            Debug.LogError("Right speaker AudioSource is missing");
