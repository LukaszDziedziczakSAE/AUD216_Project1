...
public class EnemyAudioManager : MonoBehaviour
{
    ...
    {
        StartCoroutine(VocalizationLoop(enemyName, source));
    }
    ...
    private IEnumerator VocalizationLoop(string enemyName, AudioSource source)
    {
        ...
        while (true)
        {
            ...
            if (Vector3.Distance(source.transform.position, playerTransform.position) > maxHearingDistance)
            {
